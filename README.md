...
- Polling faster than 10 s is not advised; devices have been reported to become unstable (e.g. losing CT003 connection).
 ...
 - Cumulative energy counters (`total_grid_input_energy`, `total_grid_output_energy`, `total_load_energy`, `total_pv_energy`) may be frozen at a stale value or stuck at 0 directly at the source — `ES.GetStatus` itself returns the frozen/zero value, confirmed via raw fixture captures, so this isn't something the integration can correct. Reported on Venus E 3.0 (firmware v144, fixed for grid counters after Marstek provided firmware v148 on request) and on Venus A (firmware v143/v148, load/solar energy still stuck at 0 even on v148). If you hit this, request the latest firmware build from Marstek support directly — public firmware rollouts lag behind. See [#5](https://github.com/Flodesirat/ha-marstek-local-api/issues/5).
